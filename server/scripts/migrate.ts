...
async function main() {
  ...
  await pool.query(`create table if not exists ${appSchema}.schema_migrations (filename text primary key, applied_at timestamptz not null default now())`);
  ...
  const applied = await pool.query(`select filename from ${appSchema}.schema_migrations`);
  ...
  for (const file of files) {
    ...
    try {
      ...
      await pool.query(`set local search_path to ${searchPath}`);
      await pool.query(sql);
      await pool.query(`insert into ${appSchema}.schema_migrations (filename) values ($1)`, [file]);
